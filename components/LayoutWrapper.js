    ...
    <>
      <SectionContainer header={true} className="pt-10">
        <Link href="/" aria-label="Steven Terner" className={null}>
          <div className="text-blue-600 stroke-current stroke-0 dark:stroke-1 fill-green-dark">
            <Logo />
          </div>
        </Link>
          ...
          <div className="items-center justify-around hidden w-full py-1 mr-2 font-serif font-bold text-center border-2 border-gray-900 border-solid rounded-3xl dark:border-gray-500 xl:text-2xl md:text-xl sm:flex sm:text-l">
            {headerNavLinks.map((link) => (
              <Link
                key={link.title}
                href={link.href}
                className="px-3 py-1 no-underline rounded-3xl xl:px-8 text-green-dark hover:bg-green-light hover:text-gray-900 dark:text-green-light dark:hover:text-green-lightest dark:hover:bg-green-dark"
